...
- (void)hideAnimated:(BOOL)animated {
    ...
    if (self.minShowTime > 0.f && self.showStarted) {
...
- (void)animateIn:(BOOL)animatingIn withType:(MBProgressHUDAnimation)type completion:(void (^)(BOOL finished))completion {
    ...
    if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
        [UIView animateWithDuration:0.3f delay:0.f usingSpringWithDamping:1.f initialSpringVelocity:0.f options:UIViewAnimationOptionBeginFromCurrentState animations:animations completion:completion];
        return;
    }
#endif
    [UIView animateWithDuration:0.3f delay:0.f options:UIViewAnimationOptionBeginFromCurrentState animations:animations completion:completion];
}
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeIndeterminate) {
        if (!isActivityIndicator) {
            // Update to the indeterminate indicator.
        ...
        if (!isRoundIndicator) {
            // Update to the determinate indicator.
            [indicator removeFromSuperview];
...
- (void)updateConstraints {
    ...
    if (self.square) {
        NSLayoutConstraint *square = [NSLayoutConstraint constraintWithItem:bezel attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:bezel attribute:NSLayoutAttributeWidth multiplier:1.f constant:0.f];
...
- (void)layoutSubviews {
    // There is no need to update constraints if they are going to be recreated
    // in [super layoutSubviews] due to needsUpdateConstraints being set.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        // Set if both views are visible or if there's a visible view on top
        // that doesn't have padding added relative to the current view yet.
...
- (void)setNSProgressDisplayLinkEnabled:(BOOL)enabled {
    // We're using CADisplayLink, because NSProgress can change very quickly
    // and be observing it may starve the main thread,
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    if (UIInterfaceOrientationIsLandscape(orientation)) {
        ...
        self.bounds = CGRectMake(0.f, 0.f, self.bounds.size.height, self.bounds.size.width);
    } else {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_annular) {
        ...
        CGFloat radius = (self.bounds.size.width - lineWidth) / 2.f;
        CGFloat startAngle = - ((float)M_PI / 2.f); // 90 degrees.
        CGFloat endAngle = (2.f * (float)M_PI) + startAngle;
        ...
        endAngle = (self.progress * 2.f * (float)M_PI) + startAngle;
