...
- (BOOL)isWebViewJavascriptBridgeURL:(NSURL *)url {
    if (![self isSchemeMatch:url]) return NO;
    
    return [self isBridgeLoadedURL:url] || [self isQueueMessageURL:url];
}

- (BOOL)isSchemeMatch:(NSURL *)url {
    return [url.scheme.lowercaseString isEqualToString:kNewProtocolScheme] ||
    [url.scheme.lowercaseString isEqualToString:kOldProtocolScheme];
}

- (BOOL)isQueueMessageURL:(NSURL *)url {
    NSString *host = url.host.lowercaseString;
    return [self isSchemeMatch:url] && [host isEqualToString:kWVJSBQueueMessage];
}

- (BOOL)isBridgeLoadedURL:(NSURL *)url {
    NSString *host = url.host.lowercaseString;
    return [self isSchemeMatch:url] && [host isEqualToString:kJavaSBridgeLoaded];
}
